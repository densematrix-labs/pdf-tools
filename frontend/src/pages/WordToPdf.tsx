...
import { convertWordToPdf, downloadBlob } from '../lib/pdfUtils'
...
export default function WordToPdf() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const result = await convertWordToPdf(fileState.file)
  ...
  const handleDownload = () => {
    if (fileState?.result) {
      const newName = fileState.file.name.replace(/\.(docx?|doc)$/i, '.pdf')
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 animate-fade-in">
      ...
      <div className="mt-16 prose prose-slate max-w-none">
        <h2 className="font-display">{t('tools.wordToPdf.seoTitle')}</h2>
        <p>{t('tools.wordToPdf.seoContent')}</p>
      </div>
    </div>
  )
}
